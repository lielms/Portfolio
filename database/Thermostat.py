...
#    2          Integrated PymongoDB for event logging - 2/8/2026
#------------------------------------------------------------------
...
##
## DATABASE MODULE - Import our custom database module
##
from thermostat_db import ThermostatDB
...
##
db = ThermostatDB(
    connection_string="mongodb://192.168.1.9:27017/",
...
class TemperatureMachine(StateMachine):
    ...
    ##
    ## Define the three states of the machine.
    ##
    ##  off - nothing lit up
    ...
    ## of transitioning between the three states of the
    ## thermostat
    ##
    cycle = (
        off.to(heat) |
    ...
    def processTempIncButton(self):
        ...
        # Log setpoint change to database
        db.log_state(
            state=self.current_state.id,
            temperature=floor(self.getFahrenheit()),
            setpoint=self.setPoint,
            event_type="setpoint_increase"
        )
    ...
    def updateLights(self):
        ...
        ## Begin by turning LED's off to prevent any issues
        redLight.off()
        blueLight.off()
        ...
        if self.current_state.id == "heat" and temp < self.setPoint:
            redLight.pulse()
        elif self.current_state.id == "cool" and temp > self.setPoint:
            blueLight.pulse()

    ##
    ## start the display management functionality of the thermostat
    ##
    def run(self):
    ...
    def setupSerialOutput(self):
        # Outputs state/temp/setPoint using comma delimited string for data transfer over UART
    ...
    ## Now with periodic database logging
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"{self.current_state.id}:set to {self.setPoint}F"
                altCounter = altCounter + 1
                if(altCounter >= 11):
            ...
            ## Update server and database every 30 seconds
            if((counter % 30) == 0):
...
##
## Configure green button to use GPIO 24 and to execute
...
##
## Configure Red button to use GPIO 25 and to execute
...
##
## Configure Blue button to use GPIO 12 and to execute
